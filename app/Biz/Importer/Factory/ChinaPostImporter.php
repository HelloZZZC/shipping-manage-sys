...
class ChinaPostImporter extends BaseImporter
{
    use PrepareCountry;

    protected $type = ShippingService::TYPE_CHINA_POST;
    ...
    public function execute($data)
    {
        ...
        try {
            ...
            $firstWeightRange = $this->worksheet->getCellByColumnAndRow(4, 1)->getFormattedValue();
            $firstWeightRange = explode('~', $firstWeightRange);

            $secondWeightRange = $this->worksheet->getCellByColumnAndRow(6, 1)->getFormattedValue();
            $secondWeightRange = explode('~', $secondWeightRange);

            $thirdWeightRange = $this->worksheet->getCellByColumnAndRow(8, 1)->getFormattedValue();
